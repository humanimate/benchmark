...
class ConditionVideoDataset(Dataset):
    ...
    def load_conds(self, whole_conds_path, whole_conds_video_path):
        if whole_conds_path is not None and os.path.exists(whole_conds_path):
            self.whole_conds = np.load(whole_conds_path)
            self.whole_conds = torch.from_numpy(self.whole_conds.copy()).float() / 255.0
            self.whole_conds = self.whole_conds.permute(0,3,1,2).unsqueeze(0)
