...
def get_config(config_paths):
    ...
    """
    base_conf = OmegaConf.merge(*[OmegaConf.load(str(config_path)) for config_path in config_paths])
...
class TextPoseBench_loader(Dataset):
    def __init__(self, config, load_pose_seq=True):
        self.config = config
        self.load_pose_seq = load_pose_seq
    ...
    def __getitem__(self, ndx):
        ...
        prompt = row["prompt"]
        pose_seq = np.load(self.data_path/self.vid_dir/pose_seq_name) if self.load_pose_seq else None
        data = {
            "prompt": prompt,
            "pose_seq": pose_seq,
            "codebook_index": codebook_ndx,
            # "num_frames": pose_seq.shape[0],
            "pose_seq_path": str(self.data_path/self.vid_dir/pose_seq_name),
        }
        return data
